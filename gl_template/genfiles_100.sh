...
hipstrfile=hipstr.chr20.with.1kg.filtered.vcf.gz
shapeitInput=shapeit.chr20.with.ref.vcf.gz
chr=20
...
echo "#SBATCH -t 1-00:00 # Walltime Days-Hours-Minutes" >> $fname
...
echo "timeout 5m java -Xmx32g -jar beagle.r1399.jar gtgl=str$i.snp.valid.sort.vcf.gz out=beagle.str$i.valid nthreads=4 usephase=true ped=pedigree.fam" >> $fname
...
echo "timeout 5m java -Xmx32g -jar beagle.r1399.jar gt=str$i.missing.valid.combined.vcf.gz out=beagle.str$i.phased nthreads=4 usephase=true ped=pedigree.fam" >> $fname
echo "bcftools index beagle.str$i.phased.vcf.gz" >> $fname
echo "fi" >> $fname
